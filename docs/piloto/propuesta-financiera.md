# Propuesta financiera

::: info Costo total estimado con deducciones aplicadas

El costo total del proyecto incluyendo las fases de desarrollo, implementación (6 meses), operación (12 meses) y otros costos estratégicos, junto con las deducciones aplicadas (asumiendo la colaboración institucional), es de aproximadamente **$158.450,00**.

:::

En general, el proyecto subdivide los costos en las fases de desarrollo, implementación, operación y otros costos estratégicos.

| Concepto                |        Monto |
| ----------------------- | -----------: |
| Subtotal desarrollo     |     $335.000 |
| Subtotal implementación |      $83.730 |
| Subtotal operación      |     $104.160 |
| Subtotal otros costos   |      $62.289 |
| **Total estimado**      | **$585.179** |

Las siguientes deducciones pueden ser aplicadas con la colaboración institucional en algunos aspectos claves del desarrollo, implementación y operación.

| Concepto                                                    |        Monto |
| ----------------------------------------------------------- | -----------: |
| **Total estimado preliminar**                               | **$585.179** |
| Donación en especie de Databús e Infobús                    |    -$330.000 |
| Préstamo de espacio físico en la fase de implementación     |      -$6.000 |
| Acceso a infraestructura de TI en la fase de implementación |      -$3.480 |
| Préstamo de espacio físico en la fase de operación          |     -$12.000 |
| Acceso a infraestructura de TI en la fase de operación      |      -$6.960 |
| Mantenimiento institucional de equipos                      |      -$6.000 |
| Excepción de costos estratégicos                            |     -$49.589 |
| Excepción de fondo de reposición                            |     -$10.000 |
| **Total estimado con deducciones**                          | **$158.450** |

El desglose de los costos asociados con cada una de las fases del proyecto está especificado a continuación.

## Fase de desarrollo

| Categoría            | Descripción                                | Monto    |
| -------------------- | ------------------------------------------ | -------- |
| Software             | Desarrollo de Databús e Infobús            | $330.000 |
| Preparación de datos | Generación de GTFS, levantamiento de campo | $5.000   |

**Subtotal desarrollo:** **$335.000,00**

## Fase de implementación

**Duración**: 6 meses

| Categoría                     | Descripción                                   |   Monto |
| ----------------------------- | --------------------------------------------- | ------: |
| Recursos humanos              | Salarios ($6.000/mes, 6 meses)                | $36.000 |
| Diseño gráfico y de servicios | Identidad visual, diseño UX/UI, materiales    |  $9.500 |
| Alquiler                      | Oficina / espacio ($1.000/mes, 6 meses)       |  $6.000 |
| Infraestructura en la nube    | Servidores y servicios ($580/mes, 6 meses)    |  $3.480 |
| Equipos de oficina            | Computadoras, periféricos                     | $16.000 |
| Dispositivos a bordo          | Dispositivos con GPS, interfaz y conectividad | $10.000 |
| Capacitación                  | Talleres a personal y conductores             |    $750 |
| Comunicación y difusión       | Campañas de lanzamiento y guías de usuario    |  $2.000 |

**Subtotal implementación:** **$83.730,00**

### Equipos

| Cantidad | Descripción                         | Costo unitario |   Total |
| -------- | ----------------------------------- | -------------: | ------: |
| 4        | Pantallas                           |         $1.500 |  $6.000 |
| 3        | Computadoras de alto rendimiento    |         $2.500 |  $7.500 |
| 1        | Computadora de escritorio           |         $1.000 |  $1.000 |
| 1        | Computadora de control de pantallas |         $1.500 |  $1.500 |
|          |                                     |      **Total** | $16.000 |

### Infraestructura en la nube

#### Aplicación

Servidores para la gestión de los datos del servicio de transporte público.

| Servidor  | Costo mensual |
| --------- | ------------- |
| S1        | $215          |
| S2        | $215          |
| S3        | $75           |
| S4        | $75           |
| **Total** | **$580**      |

## Fase de operación

**Duración**: 12 meses

| Categoría                  | Descripción                                            |   Monto |
| -------------------------- | ------------------------------------------------------ | ------: |
| Recursos humanos           | Salarios ($6.000/mes, 12 meses)                        | $72.000 |
| Alquiler                   | Oficina / espacio ($1.000/mes, 12 meses)               | $12.000 |
| Infraestructura en la nube | Servidores y servicios ($580/mes, 12 meses)            |  $6.960 |
| Mantenimiento de equipos   | Repuestos y servicio técnico                           |  $6.000 |
| Licencias y servicios      | Mapas, SMS gateway, APIs externas                      |      $0 |
| Conectividad dispositivos  | Planes de datos ($4/dispositivo/mes, 150 dispositivos) |   $7200 |

**Subtotal operación:** **$104.160,00**

## Otros costos estratégicos

| Categoría             | Descripción                                              |   Monto |
| --------------------- | -------------------------------------------------------- | ------: |
| Fondo de contingencia | 10% del total de desarrollo + implementación + operación | $52.289 |
| Fondo de reposición   | Sustitución de dispositivos (ciclo 3 a 5 años)           | $10.000 |

**Subtotal costos estratégicos:** **$62.289,00**
